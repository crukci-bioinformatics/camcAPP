...
##prostate-genes is a curated file with the signatures from various prostate studies
genes <- read.csv("prostate-genes.csv",header = FALSE,stringsAsFactors = FALSE)[,1]
data(cgc_67, package = "COSMIC.67")

curated.genes <- na.omit(cgc_67[,1])
##add genes from our 100 genes signature 
...
curated.genes <- c(curated.genes, genes)
curated.genes <- unique(curated.genes)
write.table(curated.genes, file="data/curated.genes.txt",row.names = FALSE,quote=FALSE)

db <- src_sqlite("data/camcap.sqlite3", create=TRUE)
db.curated <- src_sqlite("data/camcap.curated.sqlite3", create=TRUE)
...
db <- src_sqlite("data/stockholm.sqlite3", create=TRUE)
db.curated <- src_sqlite("data/stockholm.curated.sqlite3", create=TRUE)
...
db <- src_sqlite("data/taylor.sqlite3",create=TRUE)
db.curated <- src_sqlite("data/taylor.curated.sqlite3", create=TRUE)
...
expression.all <- left_join(expression, select(fd, ID, Gene))

expression <- filter(expression.all, Gene %in% curated.genes)

copy_to(db.curated, expression, temporary = FALSE, indexes = list("Gene"))
copy_to(db.curated, pd, temporary = FALSE, indexes = list("geo_accession"))
copy_to(db.curated, fd, temporary = FALSE, indexes = list("ID"))
...
db <- src_sqlite("data/varambally.sqlite3",create=TRUE)
...
db <- src_sqlite("data/grasso.sqlite3",create=TRUE)
